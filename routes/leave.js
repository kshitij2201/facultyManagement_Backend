...
router.get("/hod/:department", async (req, res) => {
  try {
    ...
    const leaves = await Leave.find({
      ...
      "employeeId name department leaveType startDate endDate reason leaveDays status"
    );
...
router.get("/principal/:department", async (req, res) => {
  try {
    const { department } = req.params;
    const departmentRegex = new RegExp(`^${department}$`, "i");
    ...
    const leaves = await Leave.find({
      ...
      "employeeId name department leaveType type startDate endDate reason leaveDays hodDecision status"
    );

    const leavesWithHodStatus = leaves.map((leave) => ({
      ...leave._doc,
      hodStatus: leave.hodDecision?.decision || "Pending",
    }));

    res.status(200).json(leavesWithHodStatus);
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: "Server error" });
  }
...
router.put("/principal/:leaveId", async (req, res) => {
  try {
    ...
    // Validate principal from Faculty model
