...
const assignCC = async (req, res) => {
  try {
    ...
    // Ensure faculty's department matches
    if (faculty.department !== department) {
      return res.status(400).json({
        success: false,
        message: `Faculty belongs to ${faculty.department}, not ${department}`,
      });
    }

    // Remove existing CC assignment for this year/semester/section/department
    ...
    // Remove any existing assignment for this faculty
    ...
    // Update faculty type to "cc" if requested
    if (updateType === "cc") {
      faculty.type = "cc";
    }
    ...
    res.status(200).json({
      ...
      data: {
        ...
        section,
        department,
        type: faculty.type, // Include updated type in response
      },
    });
  } catch (error) {
...
const getCCAssignments = async (req, res) => {
  try {
    ...
    const faculties = await Faculty.find({
      ...
      .lean();
    console.log("[GetCCAssignments] Found faculties:", faculties);
...
const deleteCCAssignment = async (req, res) => {
  try {
    ...
    // Revert type if no CC assignments remain and restoreType is provided
    if (faculty.ccAssignments.length === 0 && restoreType) {
      if (
        ...
        faculty.type = "teaching"; // Default fallback
      }
    }
